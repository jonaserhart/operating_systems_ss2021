...
#define debug(M, ...)
#else
#define debug(M, ...) fprintf(stderr, KBLU "DEBUG %s:%d:%s: " M "\n" RESET, \
...
/**
 * checks a condition and prints an error message and jumps to 'error' label if the condition is not met
 */
#define check(A, M, ...) if (!(A)) {\
    log_err(M, ##__VA_ARGS__); errno=0; goto error;}

/**
 * checks a condition and prints a debug message and jumps to 'continue' label if the condition is not met
 * handy for recursion or loops
 */
